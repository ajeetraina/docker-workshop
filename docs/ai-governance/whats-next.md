...
!!! warning
    **Roadmap content.** The CLI surface for audit and MCP governance is still landing on `sbx`. The descriptions below explain the model and point to where each pillar lives today. Validate against your installed version as features ship.
...
This is the **MCP Gateway Enterprise** layer. It sits between agent and tool, evaluating every call against the same policy engine that enforces sandbox network rules in **Network Enforcement Demo**.
...
This is **not** a polished audit surface - it's the raw daemon log. But after running the **Network Enforcement Demo** and **Filesystem Enforcement Demo** enforcement tests, you can grep it for the paste.ee, example.com, and credential-access denials to see the underlying machinery in action:
...
| --- | --- | --- | --- |
| 1. Sandbox policies | Network, filesystem, resource limits | Network proxy, mount layer | **Validated in Network Enforcement Demo + Filesystem Enforcement Demo** |
